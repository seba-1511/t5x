...
def get_optax_optimizer(optimizer_name=None, melodi_path=None, learning_rate=0.3, momentum=0.0, melodi_memory=256, melodi_model='gradient'):
    ...
    elif melodi_model == 'base-gradients-parameters-linear-multitoken':
    ...
    if 'multitoken' in melodi_model:
        if melodi_model in [
            ...
            'base-gradients-parameters-linear-multitoken',
        ]:
            # predict new parameters
    ...
    elif optimizer_name == 'adafactor-melodi-switch128-h16':
        melodi_optimizer = ChainedOptimizer((melodi_optimizer, heavyball))
        adafactor = optax.adafactor(
            learning_rate=0.3, min_dim_size_to_factor=128, decay_rate=0.8, decay_offset=-1100000,
            multiply_by_parameter_scale=False, clipping_threshold=1.0, momentum=momentum, weight_decay_rate=1e-5,
